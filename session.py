...
class Session(object):
    ...
    def train_epoch(self):
        ...
        all_penalty_loss, all_align_loss = 0., 0.

        alpha = self.env.args.alpha
        ...
        for user, pos_item, neg_item, in minibatch(users, posItems, negItems, batch_size=self.env.args.batch_size):
            ...
            # Dirichlet environment augmentation (MILK)
            mix_ratios = [[0.5, 0.5]]
            lam1, lam2 = np.random.dirichlet([alpha, alpha])
            mix_ratios.append([lam1, lam2])
            mix_ratios.append([lam2, lam1])

            env_bpr_losses = []
            for ratio in mix_ratios:
                env_user_emb, env_item_emb = self.model.get_env_emb(ratio)
                env_bpr, _ = self.bpr(env_user_emb, env_item_emb, user, pos_item, neg_item)
                env_bpr_losses.append(env_bpr)

            penalty_loss = torch.stack(env_bpr_losses).var()
            ...
            loss = bpr_loss + reg_loss + align_loss + moe_balance_loss + fusion_balance_loss + penalty_loss
        ...
        return all_loss / total_batch, all_bpr_loss / total_batch, all_reg_loss / total_batch, all_align_loss/total_batch, all_moe_balance_loss/total_batch, all_fusion_balance_loss/total_batch, all_penalty_loss / total_batch, time.time() - t
 
    def train(self, epochs):
        for epoch in range(self.env.args.ckpt_start_epoch, epochs):
            loss, bpr_loss, reg_loss, align_loss, moe_balance_loss, fusion_balance_loss, penalty_loss, train_time = self.train_epoch()
            print('-' * 30)
            print(
                f'TRAIN:epoch = {epoch}/{epochs} loss = {loss:.5f}, bpr_loss = {bpr_loss:.5f}, align_loss = {align_loss:.5f}, moe_balance_loss = {moe_balance_loss:.5f}, fusion_balance_loss = {fusion_balance_loss:.5f}, penalty_loss = {penalty_loss:.5f}, train_time = {train_time:.2f}')
            ...
            if self.env.args.log:
                self.env.train_logger.info(
                    f'EPOCH[{epoch}/{epochs}], loss = {loss:.5f}, bpr_loss = {bpr_loss:.5f}, reg_loss = {reg_loss:.5f}, penalty_loss = {penalty_loss:.5f}')
            ...
            if self.env.args.tensorboard:
                ...
                self.env.w.add_scalar(
                    f'Train/penalty_loss', penalty_loss, self.total_epoch)
